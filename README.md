...
设备有 Codex → OpenCode → 电源三个页面，左右滑手动循环切换。无人操作时仅 Codex 和 OpenCode 两页每 5 秒自动轮换；触摸期间暂停，松手后重新计时 5 秒。电源页只手动进入，停留期间不自动切页，手动离开后恢复任务页轮换。电源页右上角显示电量，页内显示正在充电 / 电池放电 / 电池待机 / 未检测到电池，以及 USB 接入状态、电池电压、USB 输入电压、VSYS 系统电压和运行时间，每秒刷新。USB 接入不等于正在充电，也不将待机误标为充满。CoreS3 没有可用的实时电流传感器，因此电流和功率明确显示“未支持”，不使用充电限流设置值或占位零值代替测量。
